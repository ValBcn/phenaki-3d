...
class TemporalSpatialAttention(nn.Module):
    def __init__(self, channels, size, num_layers=4, num_heads=4, spatial_first=True, pos_encodings=True):
        ...
        if pos_encodings:
            self.spatial_positional_encoding = nn.Parameter(torch.randn(1, 1, size * size * size, channels) / channels ** 0.5)
...
class Encoder(nn.Module):
    def __init__(self, patch_size=(5, 8, 8), input_channels=3, hidden_channels=64, size=32,
    ...
    def forward(self, image):
        ...
        image = image.view(*image.shape[:3], -1).permute(0, 2, 3, 1)  # B x T x (H x W) x C -> 1 x 21 x (16*16) x 64
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.shape[2] > 1:  # not only image training
            ...
            x = image.permute(0, 2, 1, 3, 4)  # B x C x T x H x W -> B x T x C x H x W
        return x
...
class VIVIT(nn.Module):
    def __init__(self, patch_size=(5, 8, 8), latent_size=32, c_hidden=64, c_codebook=16,
    ...
    def forward(self, image, video=None):
        (_, qe), commit_loss, _ = self.encode(image)
